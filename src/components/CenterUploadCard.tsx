...
export default function CenterUploadCard({
  ...
  onStart?: (source: { type: 'url' | 'file'; value: string; name?: string }) => void
}) {
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 bg-white/5 border border-border rounded-lg px-3 py-2">
              ...
              <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' && canSubmit) onStart?.({ type:'url', value:url.trim() }) }}
              />
            </div>
            <div className="flex items-center justify-center gap-3">
              ...
              <input ref={fileRef} type="file" hidden onChange={(e) => {
                ...
                onStart?.({ type:'file', value: URL.createObjectURL(f), name: f.name })
              }} />
            ...
            <button
                ...
                disabled={!canSubmit}
                onClick={() => onStart?.({ type:'url', value: url.trim() })}
            >
