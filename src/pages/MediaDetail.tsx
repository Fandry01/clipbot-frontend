...
import { useMedia, useTranscriptByMedia } from '../api/hooks'
...
export default function MediaDetail() {
  ...
  // Zet provider/lang naar wat jouw backend accepteert (voorbeeld: 'whisper')
  const { data: transcript } = useTranscriptByMedia(mediaId!, 'en', 'whisper')
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6">
      ...
      <div className="space-y-3">
        ...
        <TranscriptViewer
          words={(transcript?.words || []).map(w => ({ t: w.tStart, w: w.w }))}
          q={q}
      ...
      <div className="space-y-3 lg:sticky lg:top-20 h-fit">
        <Player
          ref={playerRef}
          src={media?.objectKey ? `/media/${media.objectKey}` : ''}
          poster="/src/assets/thumb1.jpg"
          aspect={aspect}
        ...
        <div className="card p-3 flex flex-wrap items-center gap-2">
          ...
          <button className="btn-ghost" onClick={() => seek(Math.max(0, currentTime - 0.2))}>-0.2s</button>
          <button className="btn-ghost" onClick={() => seek(currentTime + 0.2)}>+0.2s</button>
